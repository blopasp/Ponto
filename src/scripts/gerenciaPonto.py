...
def validacao(dado, menor:int, maior:int):
    if not dado.isdigit():
        return -1, "Favor, digite uma opção válida!"
    if menor <= int(dado) <= maior:
        return 0, ""
    else: return -1, "Favor, digite uma opção dentro da faixa!"
